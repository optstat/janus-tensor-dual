...
// -----------------------------------------------------------------------------
// Routines to copy data between host and device
// -----------------------------------------------------------------------------
SparseMatrixCSR BuildDeviceMatrixCSR(
    int rows, int cols,
    const std::vector<int>& hRowPtr,
    const std::vector<int>& hColIdx,
    const std::vector<thrust::complex<double>>& hData)
{
    SparseMatrixCSR mat;
    mat.rows_ = rows;
    mat.cols_ = cols;
    mat.nnz_  = static_cast<int>(hColIdx.size());

    cudaMalloc(&mat.row_ptr_, (rows+1)*sizeof(int));
    cudaMalloc(&mat.col_idx_, mat.nnz_*sizeof(int));
    cudaMalloc(&mat.data_,    mat.nnz_*sizeof(thrust::complex<double>));

    cudaMemcpy(mat.row_ptr_, hRowPtr.data(),
               (rows+1)*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(mat.col_idx_, hColIdx.data(),
               mat.nnz_*sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(mat.data_,    hData.data(),
               mat.nnz_*sizeof(thrust::complex<double>), cudaMemcpyHostToDevice);

    return mat;
}

void CopyDeviceMatrixToHost(
    const SparseMatrixCSR& dMat,
    std::vector<int>& hRowPtr,
    std::vector<int>& hColIdx,
    std::vector<thrust::complex<double>>& hData)
{
    hRowPtr.resize(dMat.rows_+1);
    hColIdx.resize(dMat.nnz_);
    hData.resize(dMat.nnz_);

    cudaMemcpy(hRowPtr.data(), dMat.row_ptr_,
               (dMat.rows_+1)*sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(hColIdx.data(), dMat.col_idx_,
               dMat.nnz_*sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(hData.data(),   dMat.data_,
               dMat.nnz_*sizeof(thrust::complex<double>), cudaMemcpyDeviceToHost);
}
...
TEST(CuSparseCSRAdd, EmptyMatrices)
{
    // We'll test A and B both 3x3 but zero nonzeros
    cusparseHandle_t handle;
    cusparseCreate(&handle);

    int rows=3, cols=3;
    std::vector<int> ArowPtr = {0,0,0,0};
    std::vector<int> AcolIdx = {};
    std::vector<thrust::complex<double>> Adata;
    std::vector<int> BrowPtr = {0,0,0,0};
    std::vector<int> BcolIdx = {};
    std::vector<thrust::complex<double>> Bdata;

    SparseMatrixCSR dA = BuildDeviceMatrixCSR(rows, cols, ArowPtr, AcolIdx, Adata);
    SparseMatrixCSR dB = BuildDeviceMatrixCSR(rows, cols, BrowPtr, BcolIdx, Bdata);

    // Add them with cuSPARSE
    SparseMatrixCSR dC = AddMatricesCSR_cuSPARSE(handle, dA, dB);

    // Copy result to host
    std::vector<int> CrowPtr, CcolIdx;
    std::vector<thrust::complex<double>> Cdata;
    CopyDeviceMatrixToHost(dC, CrowPtr, CcolIdx, Cdata);

    // We also do a CPU reference
    std::vector<int> CrefRowPtr, CrefColIdx;
    std::vector<thrust::complex<double>> CrefData;
    HostAddCSR(rows, cols,
               ArowPtr, AcolIdx, Adata,
               BrowPtr, BcolIdx, Bdata,
               CrefRowPtr, CrefColIdx, CrefData);

    // Compare sizes
    ASSERT_EQ(CrowPtr.size(), CrefRowPtr.size());
    ASSERT_EQ(CcolIdx.size(), CrefColIdx.size());
    ASSERT_EQ(Cdata.size(),   CrefData.size());

    // Compare row_ptr
    for (int i=0; i<(int)CrowPtr.size(); i++){
        EXPECT_EQ(CrowPtr[i], CrefRowPtr[i]) << "Mismatch in CrowPtr at i=" << i;
    }
    // Compare col, data
    for (int i=0; i<(int)CcolIdx.size(); i++){
        EXPECT_EQ(CcolIdx[i], CrefColIdx[i]) << "Mismatch in colIdx at i=" << i;
        EXPECT_DOUBLE_EQ(Cdata[i].real(), CrefData[i].real()) << "Real mismatch at i=" << i;
        EXPECT_DOUBLE_EQ(Cdata[i].imag(), CrefData[i].imag()) << "Imag mismatch at i=" << i;
    }

    // Cleanup
    cudaFree(dA.row_ptr_); cudaFree(dA.col_idx_); cudaFree(dA.data_);
    cudaFree(dB.row_ptr_); cudaFree(dB.col_idx_); cudaFree(dB.data_);
    cudaFree(dC.row_ptr_); cudaFree(dC.col_idx_); cudaFree(dC.data_);

    cusparseDestroy(handle);
}

TEST(CuSparseCSRAdd, SmallExample)
{
    cusparseHandle_t handle;
    cusparseCreate(&handle);

    // A = 3x3, row0 => col=0 val=10, row2 => col=2 val=20
    // ArowPtr=[0,1,1,2]
    int rows=3, cols=3;
    std::vector<int> ArowPtr = {0,1,1,2};
    std::vector<int> AcolIdx = {0, 2};
    std::vector<thrust::complex<double>> Adata = {
        thrust::complex<double>(10,0),
        thrust::complex<double>(20,0)
    };

    // B = 3x3, row0=> col=2 val=2, row1=> col=1 val=5, row2=> col=2 val=11
    // BrowPtr=[0,1,2,3]
    std::vector<int> BrowPtr = {0,1,2,3};
    std::vector<int> BcolIdx = {2,1,2};
    std::vector<thrust::complex<double>> Bdata = {
        thrust::complex<double>(2,0),
        thrust::complex<double>(5,0),
        thrust::complex<double>(11,0)
    };
    ...
    std::vector<int> CrefRowPtr, CrefColIdx;
    std::vector<thrust::complex<double>> CrefData;
    HostAddCSR(rows, cols, ArowPtr, AcolIdx, Adata,
               BrowPtr, BcolIdx, Bdata,
               CrefRowPtr, CrefColIdx, CrefData);
    ...
    for(int i=0; i<(int)CrowPtr.size(); i++){
        EXPECT_EQ(CrowPtr[i], CrefRowPtr[i]) << "RowPtr mismatch at i=" << i;
    }
    for(int i=0; i<(int)CcolIdx.size(); i++){
        EXPECT_EQ(CcolIdx[i], CrefColIdx[i]) << "ColIdx mismatch at i=" << i;
        EXPECT_DOUBLE_EQ(Cdata[i].real(), CrefData[i].real()) << "Real mismatch at i=" << i;
        EXPECT_DOUBLE_EQ(Cdata[i].imag(), CrefData[i].imag()) << "Imag mismatch at i=" << i;
    }
...
// Test Cases
//------------------------------------------------------------------------------
TEST(CSRMatrixReduce, EmptyMatrix)
{
    // A 3x3 matrix but 0 nnz => row_ptr=[0,0,0,0]
    int rows=3, cols=3;
    std::vector<int> ArowPtr = {0,0,0,0};
    std::vector<int> AcolIdx = {};
    std::vector<thrust::complex<double>> Adata; // empty

    // Build device matrix
    SparseMatrixCSR dA = BuildDeviceMatrixCSR(
        rows, cols, ArowPtr, AcolIdx, Adata);

    // GPU reduce
    thrust::complex<double> gpuSum = CSRMatrixReduceAll(dA);

    // CPU reference
    thrust::complex<double> cpuSum(0,0); // no nonzeros

    // Compare
    EXPECT_DOUBLE_EQ(gpuSum.real(), cpuSum.real());
    EXPECT_DOUBLE_EQ(gpuSum.imag(), cpuSum.imag());

    // Cleanup device memory
    cudaFree(dA.row_ptr_);
    cudaFree(dA.col_idx_);
    cudaFree(dA.data_);
}

TEST(CSRMatrixReduce, SmallExample)
{
    // A 3x3 with 3 nnz:
    //   row0 => col0 => 10
    //   row1 => col1 => 5+2i
    //   row2 => col2 => -7
    int rows=3, cols=3;
    std::vector<int> ArowPtr = {0,1,2,3};
    std::vector<int> AcolIdx = {0, 1, 2};
    std::vector<thrust::complex<double>> Adata = {
        thrust::complex<double>(10,0),
        thrust::complex<double>(5,2),
        thrust::complex<double>(-7,0)
    };
    ...
    // CPU reference
    thrust::complex<double> cpuSum = HostSumCSRData(Adata);
...
// Test Cases
TEST(CSRMatrixDimSum, EmptyMatrix)
{
    int rows=3, cols=4;
    // rowPtr => [0,0,0,0], no nnz
    std::vector<int> rowPtr = {0,0,0,0};
    ...
    // We'll do dim=0 => expect length=cols=4
    thrust::complex<double>* dResult=nullptr;
    cudaMalloc(&dResult, cols*sizeof(thrust::complex<double>));

    CSRMatrixSumAlongDim(dA, 0, dResult);

    // copy back
    std::vector<thrust::complex<double>> hResult(cols);
    cudaMemcpy(hResult.data(), dResult, cols*sizeof(thrust::complex<double>), cudaMemcpyDeviceToHost);

    // CPU reference
    auto cpuRef = HostDimSumCSR(rows, cols, rowPtr, colIdx, vals, 0);

    // check
    for(int c=0; c<cols; c++){
        EXPECT_DOUBLE_EQ(hResult[c].real(), cpuRef[c].real());
        EXPECT_DOUBLE_EQ(hResult[c].imag(), cpuRef[c].imag());
    }
...
TEST(CSRMatrixDimSum, SmallExample)
{
    // A = 3x3 
    //   row0 => col0 => 10
    //   row1 => col1 => 5+2i
    ...
    // rowPtr = [0,1,3,5]
    // colIdx = [0, 1,2, 0,2]
    // vals   = [10, (5,2),( -1,0), (1,0), (0,3)]
    int rows=3, cols=3;
    std::vector<int> rowPtr = {0,1,3,5};
    std::vector<int> colIdx = {0, 1,2, 0,2};
    std::vector<thrust::complex<double>> vals = {
        {10,0}, {5,2}, {-1,0}, {1,0}, {0,3}
    };

    SparseMatrixCSR dA = BuildDeviceMatrixCSR(rows, cols, rowPtr, colIdx, vals);

    // let's do dim=0 => sum across rows => get col sums => length=3
    thrust::complex<double>* dResult=nullptr;
    cudaMalloc(&dResult, cols*sizeof(thrust::complex<double>));
    ...
    // copy back
    std::vector<thrust::complex<double>> colSums(cols);
    cudaMemcpy(colSums.data(), dResult, cols*sizeof(thrust::complex<double>), cudaMemcpyDeviceToHost);

    // CPU reference
    auto cpuRef_col = HostDimSumCSR(rows, cols, rowPtr, colIdx, vals, 0);
    ASSERT_EQ((int)cpuRef_col.size(), cols);
    for(int c=0; c<cols; c++){
        EXPECT_DOUBLE_EQ(colSums[c].real(), cpuRef_col[c].real());
        EXPECT_DOUBLE_EQ(colSums[c].imag(), cpuRef_col[c].imag()) 
            << "Dim=0 mismatch at col=" << c;
    }

    // now do dim=1 => sum across columns => get row sums => length=3
    cudaMemset(dResult, 0, cols*sizeof(thrust::complex<double>())); // or free & re-alloc
    CSRMatrixSumAlongDim(dA, 1, dResult);

    std::vector<thrust::complex<double>> rowSums(rows);
    cudaMemcpy(rowSums.data(), dResult, rows*sizeof(thrust::complex<double>), cudaMemcpyDeviceToHost);

    auto cpuRef_row = HostDimSumCSR(rows, cols, rowPtr, colIdx, vals, 1);
    ASSERT_EQ((int)cpuRef_row.size(), rows);
    for(int r=0; r<rows; r++){
        EXPECT_DOUBLE_EQ(rowSums[r].real(), cpuRef_row[r].real());
        EXPECT_DOUBLE_EQ(rowSums[r].imag(), cpuRef_row[r].imag())
            << "Dim=1 mismatch at row=" << r;
    }
